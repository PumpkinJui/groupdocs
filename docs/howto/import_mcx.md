...
根据 MT 管理器提示操作。如果你*执意*要打开它，可能你需要再下载一个 Shizuku，然后用一堆开发者选项里面的内容授权，最后才能打开。所以，能用上面自动导入就别用手动导入了。

*而且，就算你靠着Shizuku成功访问了data文件夹，大多数时候你都没有办法向其中直接写入什么数据，这样你就根本没法手动导入了。*
