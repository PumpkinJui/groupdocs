...
**第六条** 自愿离开或长期不活跃且离群期间无不当行为的，可随时回到本群。  
因第八条规定由第五档处罚导致被管理员移出群聊的，原则上不得回到本群。  
由特殊原因被第五档处罚的，管理员团队可以按照第三十条的规定，过半数同意以撤销该处罚，被第五档处罚的成员在回群后需设置为第三档。
...
**第二十七条** 所有成员都应当在加入群聊之后佩戴专属的群头衔，以便于其他成员辨识身份。

**第二十八条** 群头衔展示群员称呼和违规次数。  
群头衔最多允许 6 个字符：第 1 个字符展示群成员的违规次数（如果为 0 则不记），或一个自定义的 emoji；第 2-6 个字符展示群成员的称呼。  
群头衔的称呼应保持在 1-3 个字（英文为 1-5 个字母）之内，以便于其他人称呼。

## 第四章 管理员

**第二十九条** 概述  
为促进本群成员积极为本群参与建设和贡献，制定以下管理员条例。

**第三十条** 管理员权利  
管理员在本群有包括但不限于以下所述内容的特殊权利：  
（一）允许按照群规要求撤回群员的不当发言，但必须在撤回后按群规说明理由；  
（二）允许进入管理员群聊；  
（三）允许无条件进入一切本群内非保密性质的服务器（包括开发服）；  
（四）允许在管理群内参与本群大大小小的投票决议事宜，包括但不限于更改群规、管理选拔、按第八条对违规群员施加处罚档位、或按实际情况移除处罚档位等；  
（五）允许在离群后仍然保留下载的所有文件；  
（六）允许在离群后仍然保留服务器的白名单；  
（七）在本群提供的各大服务器中成为服务器管理员。

**第三十一条** 管理员严重触犯群规  
管理员严重滥用权限的，包括但不限于以下行为，将视情节处第二档至第三档的处罚，非锁定管理的，移除管理权限：  
...
**第三十二条** 管理员触犯群规  
管理员滥用权限的，包括但不限于以下行为，将视情节处以警告至第一档的处罚：  
（一）无故撤回成员言论，或撤回违规言论但并未公开撤回理由的；  
（二）带头触犯群规以至警告或第一档处罚的；  
（三）有其他滥用职权行为，但行为轻微的。

**第三十三条** 锁定管理  
群主和晨风为锁定的管理员。为本群提供服务器对大众开放的锁定为管理员。  
如群主触犯群规需自行禁言并由全体督促。

**第三十四条** 管理选拔  
对于其他未锁定为管理的普通群员，应符合以下条件方可当选管理员。  
（一）进群半年以上；  
（二）拥有生存服白名单；  
（三）累计全部资源测试次数在原则上不少于 15 次，有重大贡献的可酌情减少次数；  
（四）从群内重要项目开始，到该重要项目结束至少参与过一次测试；  
（五）在群内表现良好，发言态度温和，没有或较少被警告或实际处罚。

**第三十五条** 管理撤销  
对于未锁定的管理员，参与测试不积极的，原则上从群内重要项目开始，到该重要项目结束都未参与测试，不在主要作者名单内的，应移除其管理权限，但在移除后可令该群员自行决定是否离开管理群。

**第三十六条** 荣誉管理  
因群聊的管理员人数限制，部分管理无法获得群聊管理员的为荣誉管理。荣誉管理享有第三十条内除第一款外的一切权利。
...
**第三十八条** 服务器的一切发言行为视为在群内的直接发言，并符合群规规定的一切具体事项。
...
**第四十条** 本群所有服务器的分配，具体信息见[服务器设定](../servers/settings)页面。

**第四十一条** 对于生存服务器，申请加入的新成员需要满足以下几点中的任意一条：  
（一）管理员；  
...
**第四十二条** 群主需要将目前正在进行的地图项目和地图进度不定期实时汇报以使全体群员知晓。

**第四十三条** 群主发布的资源将经历多个测试阶段，其他群员需要及时测试特定的资源。

**第四十四条** 群员测试后需要将发现的问题及时反馈到群内，或对应的 GitHub 仓库中的 issues 专栏中。
...
| 实验性版本 | Experimental Version | x.y - Exp z (第 x 部第 y 版的第 z 个实验性版本) | 概念性的前期版本，发布时用于展示相关概念。 | 是 | 否 |
| 快照版本 | Snapshot Version | x.y - Snapshot z (第 x 部第 y 版的第 z 个快照版本) | 即测试版本。在快照版本期间将添加多个关键功能，发布时已有相对稳定的功能，但仍旧存在许多小问题。 | 是 | 是 |
| 预发布版本 | Pre-release Version | x.y - Pre z (第 x 部第 y 版的第 z 个预发布版本) | 发布前的准备测试版本。发布时已比较稳定并已基本停止新功能的加入，更加着重于修复漏洞。需要积极的测试，以确保最终的成品不会有太多的问题。 | 是 | 是 |
| 发布候选版本 | Release Candidate Version | x.y - RC z (第 x 部第 y 版的第 z 个发布候选版本) | 发布前的准备正式版本。发布时已基本上为成品，但一旦有群员反馈了新的问题后需要及时解决并以新的发布候选版代替。地图和游戏内不显示为 RC 版本。 | 是 | 是 |
| 正式版本 | Release | x.y (第 x 部第 y 版的正式版本) | 最终成品。该版本的内容将分发到多个平台上。 | 是 | 是 (停止更新的除外) |

## 附则

**第四十六条** 对于自行有意公布自己隐私信息的，我们不对其进行保护和阻拦。

**第四十七条** 请全体成员开启防打扰，关闭特别关注，对未关闭防打扰且使用了特别关注的群成员，生活作息被打扰乃至严重打扰的，其他群成员以及被特别关注的成员不负责任。

**第四十八条** 本群将不定时清理长期不活跃成员。符合以下四条中任意一条的都可以视为是长期不活跃成员：  
...
**第四十九条** 对于所有离开本群的普通成员，必须删除自本群下载的所有私密或付费文件，不得传播这些文件。发现在未经我们允许的情况下分发我们制作的资源的，我们将追究责任。

**第五十条** 本群规最终解释权归极筑工坊及群主和管理员所有。
