...
之后一切如常即可。涉及 `npm` 和 `npx` 的命令，基本都是前面加个 p 照常用。例如原来用 `npm run start` 启动本地预览服务器，现在用 `pnpm run start` 即可。
...
如果最后这个表格仍然很工整，那就无须作任何更改。如果长得像下面这张图一样，就必须操心一下字体了。
...
理论上，等宽字体就应该把中文字符做成两个英文字母的宽度，但因为包括重视程度在内的一系列原因，很多（想了一下可能是绝大多数）等宽字体要么不支持中文，要么没有调整中文的宽度。或者，正是因为不支持中文，系统默认用了宋体或者微软雅黑，才导致宽度不一致。
