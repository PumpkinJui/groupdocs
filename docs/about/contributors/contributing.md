...
其中「类别」一般是以下列表中的一个：新增、更新、优化、修复、移除、重构、依赖、重定向、归档。其他无法被此列表完全覆盖，但使用频率较高而可以单独成一类的类别，也可以添加到此列表中，限三字以内。「具体对象」写明修改的范围即可，必要时再增加更多信息。例如：`[更新] 服务器成员表`、`[优化] 图片压缩`。

注意 `[新增] 增加服务器插图` 这种冗余写法是不受推荐的，完全可以把「增加」两字删掉。`[更新] 文档` 这种过于模糊，单纯读标题完全看不明白的写法也不推荐使用。

### 关于分支

分支是一个良好的测试工具，所属的内容将会在推送后部署在以下域名：

- `https://{branch}.groupdocs.pages.dev`
- `https://dev.docs.nekoawa.com`
