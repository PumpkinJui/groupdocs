...
在我们的实际面基经验中，我们创立了「_面基学_」并总结出了三条定律：
...
![图例](/about/offline/legend.webp)

### 上色版

![上色版](/about/offline/colored.webp)

### 黑白版（数字版）

![黑白版（数字版）](/about/offline/mono.webp)

### 技术性细节

上色版图表中的颜色使用 emoji 实现，指定使用 Windows 11 的 Segoe UI Emoji 字体进行渲染。

考虑到色觉障碍，此处的上色版图表弃用了原有的「红黄绿」配色方案，改为使用「黄绿蓝」配色方案。_而且后来发现有一种荒漠、绿洲和海洋的模拟感，非常满意。_

然而，在某些特殊情况下，这种配色方案可能仍然难以识别。如果您在查看上色版图表时无法（轻易）分辨出三种颜色，请查看黑白版（数字版）图表。两者数据完全相同，只是表现形式不同。
