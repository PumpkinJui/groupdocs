...
- 进群之后，**请不要灌水发一些无意义的内容惹人反感**，一旦有人觉得你一直在灌水，你将立刻遭受处罚。
...
- **不要卡群规的 bug**，但凡是恶意卡群规 bug 的也会受到处罚，我可不会像机器人一样循规蹈矩的执行群规内容。

- **群里可以讨论任何非违规内容！** 也就是说，你大可在群里讨论其他游戏，甚至讨论你不会做的数学题！ (但还是讨论 MC 为主会比较好 <3)
...
**第二十七条** 低等级账户（ QQ 没有太阳，也就是小于 16 级别）或敏感账户（加群时长不到 31 天），并且没有自定义头衔，如果发表敏感言论（涉政、涉恐、严重涉黄、涉赌、涉毒），可以无视其他群规，由管理员直接踢出群聊。

### 第三章 群头衔

**第二十八条** 所有成员都应当在加入群聊之后佩戴专属的群头衔。

**第二十九条** 群头衔展示群员昵称、身份和违规次数。

**第三十条** 群头衔最多允许 6 个字符，第 1 个字符展示群成员的身份，第 2 个字符展示群成员的违规次数 (如果为 0 则不记)，第 3-6 个字符展示群成员的昵称。

**第三十一条** 群头衔的身份以特殊符号记，申请自定义身份的群员必须保持无违规记录。具体符号如下表所示：
...
**第三十二条** 群头衔的昵称应保持在 2-3 个字 (英文为 3-4 个字母) 之内，以便于其他人称呼。

### 第四章 管理员

**第三十三条** 群主认定为特殊的管理员，锁定为量筒，如群主触犯群规需自行禁言并由全体督促。

**第三十四条** 极筑工坊官方账号锁定为荣誉管理。

**第三十五条** 提供服务器对大众开放的锁定为管理。

**第三十六条** 对其他未锁定为管理的群员，积极乐观，态度友好端正，积极测试地图、汇报 bug、分享资源或建设服务器的，在经过其他管理的多数同意的情况下可以成为管理。

**第三十七条** 管理员与普通成员的比例宜保持在 1:8~1:10 之间。

**第三十八条** 任何未锁定为管理的管理员，当符合全部以下条件的撤销管理权限：  
（一）半年或以上未测试组件的；  
（二）半年或以上未辅助制作资源的；  
（三）没有任何其他贡献且长时间不发表言论的。

**第三十九条** 管理员可以进入非保密性的开发服务器。

**第四十条** 管理员可以在主动退群后保留在群内下载的所有文件。

**第四十一条** 管理员可以在主动退群后保留所有服务器的白名单。

**第四十二条** 管理员可以更改群规和群文档，但必须在更改群规之后通知全体成员。

**第四十三条** 管理员可以、并有义务撤回成员的不当发言，但必须在撤回后说明理由。

**第四十四条** 管理员严重滥用权限的，包括但不限于以下行为，将视情节处第二档至第三档的处罚，非锁定管理的，移除管理权限：  
...
**第四十五条** 管理员滥用权限的，包括但不限于以下行为，将视情节处以警告至第一档的处罚：  
...
**第四十六条** 服务器的一切发言行为视为在群内的直接发言，并符合群规规定的一切具体事项。

**第四十七条** 下文所有服务器均采用白名单机制，所有未进群且未经特殊允许的成员必须进入群聊且在登记表格中由群主登记后才允许进入相关服务器。被处罚 3 档以上的成员不得申请登记，也不得申请撤销移除服务器黑名单。  
有需求进入服务器的成员，需要按照群昵称规则「XBoxID 其他内容」的格式在群昵称中事先声明自己的 XBoxID 。

**第四十八条** 本群所有服务器的分配，具体信息见[服务器设定](server_settings.md)页面。服务器的地址和端口简表如下：
...
**第四十九条** 对于生存服务器而言，所有成员有权利推荐一个资源包加入到服务器中，但必须经由至少 3 位常进服务器的成员投票，每位成员可投赞成票、反对票和中立票，赞成票必须至少多于反对票 3 票才允许加入该资源包。群主不参与投票。

**第五十条** 对于生存服务器而言，申请加入的新成员需要满足以下几点中的任意一条：  
（一）管理员；  
（二）积极测试地图，贡献较大的；  
（三）管理员推荐，且表现良好的；  
...
**第五十一条** 群主需要将目前正在进行的地图项目和地图进度不定期实时汇报以使全体群员知晓。

**第五十二条** 群主发布的资源将经历多个测试阶段，其他群员需要及时测试特定的资源。

**第五十三条** 群员测试后需要将发现的问题及时反馈到群内，或对应的 GitHub 仓库中的 issues 专栏中。
...
**第五十五条** 对于自行有意公布自己隐私信息的，我们不对其进行保护和阻拦。

**第五十六条** 请全体成员开启防打扰，关闭特别关注，对未关闭防打扰且使用了特别关注的群成员，生活作息被打扰乃至严重打扰的，其他群成员以及被特别关注的成员不负责任。

**第五十七条** 本群将不定时清理长期不活跃成员。符合以下四条中任意一条的都可以视为是长期不活跃成员：  
...
**第五十八条** 对于所有离开本群的普通成员，必须删除自本群下载的所有私密或付费文件，不得传播这些文件。发现在未经我们允许的情况下分发我们制作的资源的，我们将追究责任。

**第五十九条** 本群规最终解释权归极筑工坊及群主和管理员所有。
